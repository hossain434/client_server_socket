...
public class BinaryTreeForInput<Key extends Comparable<Key>> {
    private Node root;             // root of BST
    private int duplicates = 0;
    private File file;

    public BinaryTreeForInput() {
    ...
    private Node put(Node x, Key key) {
        ...
        int inputCompare = key.compareTo(x.key);
        if      (inputCompare < 0) x.left  = put(x.left,  key);
        else if (inputCompare > 0) x.right = put(x.right, key);
        else     duplicates++;
