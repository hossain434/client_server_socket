...
public class SetupFiveClientServers {
    private ServerSocket serverSocket;
    private ExecutorService executorService = Executors.newFixedThreadPool(5);
    private BlockingQueue<Integer> queue;

    // blockingQueue used to keep process thread safe.
    public SetupFiveClientServers(BlockingQueue<Integer> blockingQueue) {
    ...
    public void start(int port) {
        try {
            ...
            AddQueue queuer = new AddQueue(queue);
    ...
    public static void main(String[] args) {
        ...
        SetupFiveClientServers server = new SetupFiveClientServers(queue);
        server.start(4000);
    }
}
